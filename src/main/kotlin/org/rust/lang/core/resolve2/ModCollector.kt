    ...
    private fun tryCollectChildModule(item: ItemLight, stub: RsNamedStub): ModData? {
        ...
        val (childMod, hasMacroUse, pathAttribute) = when (stub) {
            ...
            is RsModDeclItemStub -> {
                val childModPsi = stub.resolveAndGetPossiblePaths(modData, project, defMap.missedFiles) ?: return null
...
private fun RsModDeclItemStub.resolveAndGetPossiblePaths(
    containingModData: ModData,
    project: Project,
    missedFiles: MutableList<Path>
): RsFile? {
    val (parentDirectory, names) = if (pathAttribute === null) {
        ...
        val names = arrayOf("$name.rs", "$name/mod.rs")
    ...
    val virtualFiles = names.mapNotNull { parentDirectory.virtualFile.findFileByMaybeRelativePath(it) }
    ...
    if (virtualFiles.isEmpty()) {
        for (name in names) {
            val path = parentDirectory.virtualFile.pathAsPath.resolve(name)
            missedFiles.add(path)
        }
    }
