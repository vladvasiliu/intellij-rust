...
class RsSdkDetailsDialog(
    ...
    private fun apply() {
        ...
        if (modifiedSdks.isNotEmpty()) {
            val listener = project?.messageBus?.syncPublisher(RsSdkAdditionalData.RUST_ADDITIONAL_DATA_TOPIC)
    ...
    private fun updateVersionString(modificator: SdkModificator) {
        ...
        val toolchain = RustToolchain(homePath, data.rustupOverride)
        val versionInfo = effectiveProject.computeWithCancelableProgress("Fetching rustc version...") {
            toolchain.queryVersions()
        }
        modificator.versionString = versionInfo.rustc?.semver?.parsedVersion
    }
